...
// inline ad keys + any trigger_* + per-screen native
// (control_onboard_screen_* + control_language_screens) + legacy control_native_*
// (delete-only). Anything else is refused.
...
export function discoverTriggerKeys(template: RemoteConfigTemplate): string[] {
  return collectManaged(template, (baseKey) =>
    baseKey.startsWith(TRIGGER_PREFIX),
  );
}

// All control_onboard_screen_* params on the template.
export function discoverOnboardScreenKeys(
  template: RemoteConfigTemplate,
): string[] {
  return collectManaged(template, (baseKey) =>
    baseKey.startsWith(ONBOARD_SCREEN_PREFIX),
  );
}

// Legacy control_native_* params still on the template (for cleanup).
export function discoverObsoleteNativeKeys(
  template: RemoteConfigTemplate,
): string[] {
  return collectManaged(template, (baseKey) =>
    baseKey.startsWith(OBSOLETE_NATIVE_PREFIX),
  );
}

// InlineAd params: control_game_footer + control_home_footer.
export function discoverInlineAdKeys(template: RemoteConfigTemplate): string[] {
  return collectManaged(template, (baseKey) =>
    (INLINE_AD_KEY_LIST as readonly string[]).includes(baseKey),
  );
}
